...
class TZI_Checker:
    ...
    def select_folder(self):
        while True:
            ...
            folder = input(f"{ColorCyan}Enter path to folder containing results: {ColorReset}").strip()
            if not folder or not os.path.isdir(folder):
                print(f"{ColorRed}Invalid folder. Try again.{ColorReset}")
                time.sleep(1)
                continue
            results_path = None
            for root, dirs, _ in os.walk(folder):
                for d in dirs:
                    if d.lower() == "results":
                        ...
                        break
                if results_path: break
            if not results_path:
                print(f"{ColorRed}No 'results' folder found.{ColorReset}")
                time.sleep(1)
                continue
            return results_path

    def extract_emails_from_results(self, results_path):
        ...
        for root, _, files in os.walk(results_path):
            for f in files:
                if f.lower().endswith('.txt'):
                    fname = f.lower()
                    if "hanging" in fname and "hit" in fname:
                            ...
                            for line in fh:
                                ...
                                if match:
                                    ...
                                    if email not in seen:
                                        seen.add(email)
                                        self.found_accounts.append((line, email, match.group(3)))
        ...
        try:
            ...
            try:
                ...
                for uid in uids:
                    ...
                    body_text = body_div.get_text(separator="\n", strip=True) if body_div else "(No body found or empty)"
                    body_text = re.sub(r'\n{3,}', '\n\n', body_text).strip()
    ...
    def run(self):
        os.system('cls' if os.name == 'nt' else 'clear')
        results_path = self.select_folder()
        count = self.extract_emails_from_results(results_path)
        os.system('cls' if os.name == 'nt' else 'clear')
        print(f"{ColorCyan}{ColorBold}TZI AML CHECKER - BY Chuborn{ColorReset}\n")
        print(f"found {count} aml in results folder\n")
